...
lib.addCommand({ 'additem', 'giveitem' }, {
	...
	params = {
		{ name = 'target', type = 'number', help = 'Nguời chơi bạn muốn give đồ' },
		{ name = 'item', type = 'string', help = 'Tên vật phẩm [Mã CODE]' },
	...
	local tPlayer = ESX.GetPlayerFromIdcard(args.target)
	if not tPlayer then
		return TriggerClientEvent("esx:showNotification", source, "Người chơi không online!")
	end
	if item then
		local inventory = Inventory(tPlayer.source) --[[@as OxInventory]]
